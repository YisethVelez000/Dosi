...
export const ListarProducto = async (filters: Partial<Filters & Pagination & Sorting>) => {
  try {
    // Asegúrate de que sortingOrder siempre sea una cadena de texto
    const validSortingOrder = filters.sortingOrder || 'asc' // Asigna un valor predeterminado si sortingOrder es nulo

    // Realizar la solicitud GET a la API
    const response = await axios.get('https://apinodedvp.onrender.com/productos', {
      params: { ...filters, sortingOrder: validSortingOrder },
    })
    ...
    if (filters.search) {
      ...
      filteredData = data.filter((producto: Producto) => {
        return (
          ...
          producto.PrecioProducto.toString().includes(searchTerm) // Cambié la coma por ||
        )
      })
    }
...
export const EditarProducto = async (producto: Producto) => {
  try {
    ...
    await sleep(1000)
    const response = await axios.put(`https://apinodedvp.onrender.com/${producto.IdProducto}`, producto)
