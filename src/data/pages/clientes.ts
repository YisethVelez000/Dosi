...
export const getClientes = async (filters: Partial<Filters & Pagination & Sorting>) => {
  ...
  if (filters.isActive) {
    console.log('Filtrando por estado:', filters.isActive)
    filteredData = filteredData.filter((cliente: Cliente) => {
      return cliente.Usuario.EstadoUsuario.toLowerCase() === (filters.isActive ?? '').toLowerCase()
    })
  }
  ...
  if (filters.search) {
    ...
    filteredData = filteredData.filter((cliente: Cliente) => {
      return (
        cliente.Usuario.Nombre.toLowerCase().includes(searchTerm) ||
        cliente.Usuario.Apellidos.toLowerCase().includes(searchTerm) ||
        cliente.NumeroDocumento.toLowerCase().includes(searchTerm) ||
        cliente.Usuario.CorreoElectronico.toLowerCase().includes(searchTerm) ||
        cliente.Telefono.toLowerCase().includes(searchTerm)
      )
    })
  }

  // Mostrar en consola la data filtrada
