<template>
  ...
  <VaModal v-model="isAnularModalVisible" :ok-text="'Anular'" :cancel-text="'Cancelar'" @ok="anularCompra">
    <template #title>Anular Compra</template>
    <template #default>
      <VaTextarea
        v-model="MotivoAnulacion"
        label="Motivo de Anulación"
        :rules="[(valor) => !!valor || 'Motivo de anulación es obligatorio']"
      />
    </template>
  </VaModal>
...
const anularCompra = async () => {
  if (!MotivoAnulacion.value) {
    return // No hacer nada si el campo es inválido
  }
