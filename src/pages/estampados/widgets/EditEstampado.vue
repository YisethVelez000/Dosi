...
onMounted(async () => {
  ...
  // Extraer los IDs de las ubicaciones seleccionadas para el v-model de los checkboxes
  ubicacionesSeleccionadas.value = estampado.value.EstampadosUbicaciones.map((u) => u.Ubicacione.IdUbicacion)
})
...
const onSave = async () => {
  if (form.validate()) {
    try {
      const estampadoData = {
        ...estampado.value,
        EstampadosUbicaciones: ubicacionesSeleccionadas.value.map((id) => ({
          IdUbicacion: id,
        })),
      }
