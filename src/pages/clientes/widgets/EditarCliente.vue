...
// Propiedad para controlar la validez del formulario
...
const numeroDocumentoRules: ((v: string) => boolean | string)[] = [
  ...
  (v: string) => /^[^\s]+(\s+[^\s]+)*$/.test(v) || 'El nombre no puede tener espacios en blanco al inicio ni al final',
...
<template>
  ...
  <VaCard>
    <VaCardContent>
      <VaForm ref="formRef" class="flex-col justify-start items-start gap-4 inline-flex w-full">
        <div class="self-stretch flex-col justify-start items-start gap-4 flex">
          ...
          <div class="flex justify-end gap-4 w-full">
            ...
            <VaButton :disabled="!formIsValid" @click="onSave">Guardar</VaButton>
          </div>
        </div>
      </VaForm>
    </VaCardContent>
